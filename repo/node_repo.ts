...
export class NodeRepo {
  ...
  static async get_list(id: string): Promise<Dir []> {
    const db = await getDB();
    const res: Dir [] = await db.select().from(dir).where(eq(dir.parent_id, id));
  ...
  static async is_name_already_present(parent_id: string, new_name: string): Promise<boolean> {
    const db = await getDB();
    const res: Dir [] = await db.select().from(dir).where(and(eq(dir.parent_id, parent_id), eq(dir.node_name, new_name)));
  ...
  static async remove_by_node_id(node_id: string): Promise<Dir []> {
    ...
    const res: Dir [] = await db.select().from(dir).where(eq(dir.parent_id, node_id));
    return res;
  }

  static async is_repo_not_present(node_id: string): Promise<boolean> {
    const db = await getDB();
    const res: Dir [] = await db.select().from(dir).where(and(eq(dir.node_id, node_id), eq(dir.parent_id, dir.owner_id)));
  ...
  static async get_node(node_id: string): Promise<Dir> {
    const db = await getDB();
    const res: Dir [] = await db.select().from(dir).where(eq(dir.node_id, node_id));
  ...
  static async get_repo_list(user_id: string): Promise<Dir []> {
    const db = await getDB();
    const res: Dir [] = await db.select().from(dir).where(and(eq(dir.parent_id, user_id), eq(dir.owner_id, dir.parent_id)));

    return res;
  }

  static async get_folder_list(folder_id: string): Promise<Dir []> {
    const db = await getDB();
    const res: Dir [] = await db.select().from(dir).where(and(eq(dir.parent_id, folder_id)));

    return res;
  }

  static async get_public_repo_list(user_id: string): Promise<Dir []> {
    const db = await getDB();
    const res: Dir [] = await db.select().from(dir)
